---
title: "Instructions for pre-processing"
...
* **populate_word_element( )** returns and XML node containing the original attributes from the target node in the input file and also the new attributes generated by this script.
    + **y <- as.numeric(x["id"])** returns a numeric vector of length 1 containing the value of the target word's id attribute.  This value will be used to index **node.list** to select new attributes for the target node.
    + **word.xml <- xmlNode("word")** creates an XML node object, **word.xml**. The XML package function **xmlNode( )** takes as its argument the name of the resulting XML node in the XML structure (here "word").
    + **if (node.list$Ellipsis[y] == FALSE) {...]** selects for an input token that is not an ellipsis. This step is necessary because in the original input file ellipsis nodes have a slightly different set of attributes than regular tokens. If the element of the appropriate element in the **Ellipsis** vector in **node.list**  is FALSE, the input is not an ellipsis and the indicated code block is applied.
        + **word.xml <- addAttributes(word.xml, id = x["id"], form = x["form"], ...** populates **word.xml** with the attributes given as arguments to the **addAttributes( )** function from the XML package.
            + **addAttributes( )** takes as its first argument the XML node to which the attributes are to be appended. Subsequent arguments give the attributes to be added. These arguments take the form NAME = VALUE; thus, **id = x["id"]** creates an attribute called "id" and assigns to it the value of the id attribute of the input token **x**.
   + **else {...}** selects for an input node that is an ellipsis and applies the specified code block.
        + **word.xml <- addAttributes(word.xml, id = x["id"], form = x["form"], ...** populates **word.xml** with the attributes given as arguments to the **addAttributes( )** function from the XML package. The attributes as the same as for the non-ellipsis node with the following additions.
            + **insertion_id = x["insertion_id"]** and **artificial = x["artificial"]:** both attribute types are specific to ellipses.
    + **return(word.xml)** passes the populated XML node to the matrix code.  
    
    
### End of User-Defined Functions for XML Attributes

***
***
***

### XSLT Script to Expand Morphology Abbreviations and Link Multiple Syntactic "Generations"

```{r eval = FALSE}

# Note that this is not R code. It is XSLT and should be treated appropriately.
...
##### Note: in these comments open and closing angle brackets are omitted.

* The purpose of this XSLT script is to: 
    + extract input attribute values and give these values to new attributes with names that will be easier to distinguish in subsequent stages of the process 
    + expand morphology annotation to an easily readable form
    + add attributes representing the releavnt features of the syntactic ancestors of each word.
    
* **xsl:output method="xml" indent="yes"** sets the output of the XSLT to produce and XML file with  indentations according to hierarchical structure for clarity.

* **xsl:template match="node() | @*"** introduces the identity transformation. This line uses wild card symbols as arguments to the **match=""** argument; the wild cards apply this template to the XML root, whatever thay may be. 
    + **xsl:copy** performs a shallow copy. This term means that this command copies only the node refered to in the template **match** attribute; it does not copy attributes or child elements. Without further modification, this template would copy only the root element of the file.
        + **xsl:apply-templates select="node() | @*"** identifies a set of attributes and nodes as specified in the command's **select** attribute. It then looks for templates whose **match** attribute refers to members of that set and applies the appropriate template. Here, the **select** attribute is set, using wild cards, to all nodes and all attributes. Because the **match** attribute for the identity transformation itself is in the set defined by **select**, the **xsl:copy** command in the identity transformation is applied recursively to all descendant nodes of the root and all attributes.
        + **xsl:apply-templates** applies matched templates in such a way that the output is changed as directed and other input is passed on unchanged.

* **xsl:template match="word[not(@lemma)]"** is a template to handle word elements lacking the **lemma** attribute entirely (not just a missing value). Some older treebank files have such incomplete elements as the result of ellipsis. A missing **lemma** attribute will break a later stage of the code.
    + **match="word[not(@lemma)]"** uses the index brackets **[...]** to specify particular word elements. The desired selection is given using the XQuery **not(  ) function**. The expression **[not(@lemma)]** means "where it is not the case that the attribute lemma exists." 
    + **&lt;word&gt;** creates a new word element.
        + **xsl:attribute name="lemma"** creates for the word element a new attribute named **lemma**. It is populated by the value **NA**.
        + **xsl:copy-of select="@\*"** implements a deep copy, duplicating all attributes and nodes indicated by the **select** attribute. Here wild cards select all attributes of the input element.


* **xsl:template match="word[not(@postag)]"** is a template to handle word elements lacking the **postag** attribute entirely (not just a missing value). Some older treebank files have such incomplete elements as the result of ellipsis. A missing **postag** attribute will break a later stage of the code.
    + **match="word[not(@postag)]"** uses the index brackets **[...]** to specify particular word elements. The desired selection is given using the XQuery **not(  ) function**. The expression **[not(@postag)]** means "where it is not the case that the attribute postag exists." 
    + **&lt;word&gt;** creates a new word element.
        + **xsl:attribute name="postag"** creates for the word element a new attribute named **postag**. It is populated by the value **NA**.
        + **xsl:copy-of select="@\*"** implements a deep copy, duplicating all attributes and nodes indicated by the **select** attribute. Here wild cards select all attributes of the input element.
     
     
     
* **xsl:template match="word[not(@form)]"** is a template to handle word elements lacking the **form** attribute entirely (not just a missing value). Some older treebank files have such incomplete elements as the result of ellipsis. A missing **form** attribute will break a later stage of the code.
    + **match="word[not(@form)]"** uses the index brackets **[...]** to specify particular word elements. The desired selection is given using the XQuery **not(  ) function**. The expression **[not(@form)]** means "where it is not the case that the attribute form exists." 
    + **&lt;word&gt;** creates a new word element.
        + **xsl:attribute name="form"** creates for the word element a new attribute named **form**. It is populated by the value **NA**.
        + **xsl:copy-of select="@\*"** implements a deep copy, duplicating all attributes and nodes indicated by the **select** attribute. Here wild cards select all attributes of the input element.


 
 
* **xsl:template match="word"** is a template to create new attribute to serve as variables for author analysis. This template will apply to all word elements not specified in the preceding templates. The first step is to set up a series of variables to be used to create the desired attributes.
    + **xsl:variable name="pos" select="substring(./@postag, 1, 1)"** creates a variable named **pos** and populates it with a letter indicating the part of speech of the current word element.
        + **select="substring(./@postag, 1, 1)** assigns the value of the variable. The XQuery function **substring( )** returns the segment of a string indicated by its arguments.
            + The first argument specifies the input string, here **./@postag** indicating the value of the **postag** attribute of the current word element. The second argument specifies the position of the string at which to begin extraction.  The third argument specifies the length of the string to be extracted. Thus, the arguments **1, 1** return the first character in the **postag** attribute, which is the symbol for part of speech.
    + **xsl:variable name="person" select="substring(./@postag, 2, 1)"** creates a variable named **person** and populates it with a letter indicating the grammatical person of the current word element.
    + **xsl:variable name="number" select="substring(./@postag, 3, 1)"** creates a variable named **number** and populates it with a letter indicating the grammatical number of the current word element.
    + **xsl:variable name="tense" select="substring(./@postag, 4, 1)"** creates a variable named **tense** and populates it with a letter indicating the grammatical tense of the current word element.
    + **xsl:variable name="mood" select="substring(./@postag, 5, 1)"** creates a variable named **mood** and populates it with a letter indicating the grammatical mood of the current word element.
    + **xsl:variable name="voice" select="substring(./@postag, 6, 1)"** creates a variable named **mood** and populates it with a letter indicating the grammatical mood of the current word element.
    + **xsl:variable name="gender" select="substring(./@postag, 7, 1)"** creates a variable named **gender** and populates it with a letter indicating the grammatical gender of the current word element.
    + **xsl:variable name="case" select="substring(./@postag, 8, 1)"** creates a variable named **case** and populates it with a letter indicating the grammatical case of the current word element.
    + **xsl:variable name="degree" select="substring(./@postag, 9, 1)"** creates a variable named **degree** and populates it with a letter indicating the grammatical degree of the current word element.
    
### Note that no single word has a value in all grammatical categories indicated by these variables.  However,  it is assumed to be more efficient to apply all categories to all words than to write separate code for each part of speech.  Inapplicable variables can be weeded out later in the process. 
 
 
 + **&lt;word&gt;** creates a word element to be populated be the nested code.
 
 
       + **xsl:copy-of select="@\*"** replicates all attributes in the input node.
       + **xsl:attribute name="self-relation** creates an attribute named "self-relation" to store the value of the **relation** attribute of the input node.
           + **xsl:value-of select="./@relation"** returns the value of the expression specified in the **select** attribute. Here, **.** indicates the current node and **/@relation** indicates the attribute named "relation."
       + **xsl:attribute name="self-depdist"** creates an attribute named "self-depdist" to store the dependency distance of the input node.
           + **xsl:value-of select="./@DepDist"/** returns the value of the **DepDist** attribute of the input node.
       + **xsl:attribute name="self-vertexDegree"** creates an attribute named "self-vertexDegree" to store the vertex degree of the input node.
           + **xsl:value-of select="./@Degree"** returns the value of the **Degree** attribute of the input node.
       
       
       
       + **xsl:choose** applies one or more condition tests as specified in nested nodes. The **choose** element in analogous to the **if ( ) {...}** command in R and similar languages.
           + **xsl:when test="&#36;pos='d'"** specifies a condition (there may be more than one condition) for the matrix **choose** element. The condition is specified through the value of the expression in the **test** attribute. Here, the test is set to **&#36;pos='d'**.  This **when** element returns TRUE when the value of the **pos** variable is "**d**", the symbol for **adverb** in the AGLDT (Ancient Greek and Latin Dependency Treebank).
             + **xsl:attribute name="self-morph-pos"** creates an attribute in the matrix word element with the name indicated in the **name** attribute. Here, the name is **self-morph-pos**. This kind of extended name was chosen to help disambiguate the new variables created in our study. For example, "self" makes it immediately clear that this value belongs to the target word and not to an ancestor or descendant. Likewise, "morph" makes it clear that, for example, "degree" is used in the morphological sense used in the comparision of adjectives and adverbs, not in the sense common in the study of graphs.
                 + The value of the **name** attribute is given by the nested text string, here **"adverb**.
          + **xsl:when test="&#36;pos='r'"** returns TRUE when the value of the **pos** variable is "**r**", the symbol for **preposition** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string. 
          + **xsl:when test="&#36;pos='l'"** returns TRUE when the value of the **pos** variable is "**l**", the symbol for **definite article** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string.
          + **xsl:when test="&#36;pos='v'"** returns TRUE when the value of the **pos** variable is "**v**", the symbol for **verb** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string. 
          + **xsl:when test="&#36;pos='a'"** returns TRUE when the value of the **pos** variable is "**a**", the symbol for **adjective** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string. 
          + **xsl:when test="&#36;pos='p'"** returns TRUE when the value of the **pos** variable is "**p**", the symbol for **pronoun** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string. 
          + **xsl:when test="&#36;pos='n'"** returns TRUE when the value of the **pos** variable is "**n**", the symbol for **noun** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string. 
          + **xsl:when test="&#36;pos='c'"** returns TRUE when the value of the **pos** variable is "**c**", the symbol for **conjuntion** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string. 
          + **xsl:when test="&#36;pos='u'"** returns TRUE when the value of the **pos** variable is "**u**", the symbol for **punctuation** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string.
          + **xsl:otherwise** returns TRUE when none of the previous conditions are met. This element is necessary to handle cases in which the pos value is blank or otherwise abnormal in the input XML. The nested **xsl:attribute** node assigns a value of the text string **"NA"** in this case.
          
          
     + The next **xsl:choose** element controls for the grammatical category of **number**.  
         + **xsl:when test="&#36;number='s'"** returns TRUE when the value of the **number** variable is "**s**", the symbol for **singular** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string.
         + **xsl:when test="&#36;number='p'"** returns TRUE when the value of the **number** variable is "**p**", the symbol for **plural** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string.
         + **xsl:when test="&#36;number='d'"** returns TRUE when the value of the **number** variable is "**d**", the symbol for **dual** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string.
         + **xsl:otherwise** returns TRUE when none of the previous conditions are met. This element is necessary to handle cases in which the category number is inappropriate, or when the input value is missing or otherwise abnoral in the input. The nested **xsl:attribute** node assigns a value of the text string **"NA"** in this case.
         
         
     + The next **xsl:choose** element controls for the grammatical category of **tense**.  
         + **xsl:when test="&#36;tense='p'"** returns TRUE when the value of the **tense** variable is "**p**", the symbol for **present** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string.
         + **xsl:when test="&#36;tense='i'"** returns TRUE when the value of the **tense** variable is "**i**", the symbol for **imperfect** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string.
         + **xsl:when test="&#36;tense='a'"** returns TRUE when the value of the **tense** variable is "**a**", the symbol for **aorist** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string.
         + **xsl:when test="&#36;tense='r'"** returns TRUE when the value of the **tense** variable is "**r**", the symbol for **perfect** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string.
         + **xsl:when test="&#36;tense='l'"** returns TRUE when the value of the **tense** variable is "**l**", the symbol for **pluperfect** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string.
         + **xsl:when test="&#36;tense='f'"** returns TRUE when the value of the **tense** variable is "**f**", the symbol for **future** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string.
         + **xsl:when test="&#36;tense='t'"** returns TRUE when the value of the **tense** variable is "**t**", the symbol for **future perfect** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string.
         + **xsl:otherwise** returns TRUE when none of the previous conditions are met. This element is necessary to handle cases in which the category number is inappropriate, or when the input value is missing or otherwise abnoral in the input. The nested **xsl:attribute** node assigns a value of the text string **"NA"** in this case.
 
 
    + The next **xsl:choose** element controls for the grammatical category of **mood**. 
         + **xsl:when test="&#36;mood='i'"** returns TRUE when the value of the **mood** variable is "**i**", the symbol for **indicative** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string.
         + **xsl:when test="&#36;mood='s'"** returns TRUE when the value of the **mood** variable is "**s**", the symbol for **subjunctive** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string.
         + **xsl:when test="&#36;mood='o'"** returns TRUE when the value of the **mood** variable is "**o**", the symbol for **optative** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string.
         + **xsl:when test="&#36;mood='n'"** returns TRUE when the value of the **mood** variable is "**n**", the symbol for **infinitive** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string.
         + **xsl:when test="&#36;mood='p'"** returns TRUE when the value of the **mood** variable is "**p**", the symbol for **participle** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string.
         + **xsl:otherwise** returns TRUE when none of the previous conditions are met. This element is necessary to handle cases in which the category number is inappropriate, or when the input value is missing or otherwise abnoral in the input. The nested **xsl:attribute** node assigns a value of the text string **"NA"** in this case. 
         
         
             
               
     + The next **xsl:choose** element controls for the grammatical category of **voice**. 
         + **xsl:when test="&#36;voice='a'"** returns TRUE when the value of the **voice** variable is "**a**", the symbol for **active** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string.
         + **xsl:when test="&#36;voice='p'"** returns TRUE when the value of the **voice** variable is "**p**", the symbol for **passive** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string.
         + **xsl:when test="&#36;voice='m'"** returns TRUE when the value of the **voice** variable is "**m**", the symbol for **middle** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string.
         + **xsl:when test="&#36;voice='e'"** returns TRUE when the value of the **voice** variable is "**e**", the symbol for **deponent** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string.
         + **xsl:otherwise** returns TRUE when none of the previous conditions are met. This element is necessary to handle cases in which the category number is inappropriate, or when the input value is missing or otherwise abnoral in the input. The nested **xsl:attribute** node assigns a value of the text string **"NA"** in this case.
         
         
     + The next **xsl:choose** element controls for the grammatical category of **gender**. 
         + **xsl:when test="&#36;gender='m'"** returns TRUE when the value of the **gender** variable is "**m**", the symbol for **masculine** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string.
         + **xsl:when test="&#36;gender='f'"** returns TRUE when the value of the **gender** variable is "**f**", the symbol for **feminine** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string.
         + **xsl:when test="&#36;gender='n'"** returns TRUE when the value of the **gender** variable is "**n**", the symbol for **neuter** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string.
         + **xsl:otherwise** returns TRUE when none of the previous conditions are met. This element is necessary to handle cases in which the category number is inappropriate, or when the input value is missing or otherwise abnoral in the input. The nested **xsl:attribute** node assigns a value of the text string **"NA"** in this case.
         
     + The next **xsl:choose** element controls for the grammatical category of **case**.
         + **xsl:when test="&#36;case='m'"** returns TRUE when the value of the **case** variable is "**n**", the symbol for **nominative** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string.         
         + **xsl:when test="&#36;case='g'"** returns TRUE when the value of the **case** variable is "**g**", the symbol for **genitive** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string.
         + **xsl:when test="&#36;case='d'"** returns TRUE when the value of the **case** variable is "**d**", the symbol for **dative** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string. 
         + **xsl:when test="&#36;case='a'"** returns TRUE when the value of the **case** variable is "**a**", the symbol for **accusative** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string. 
         + **xsl:when test="&#36;case='b'"** returns TRUE when the value of the **case** variable is "**b**", the symbol for **ablative** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string. 
         + **xsl:when test="&#36;case='v'"** returns TRUE when the value of the **case** variable is "**v**", the symbol for **vocative** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string. 
         + **xsl:otherwise** returns TRUE when none of the previous conditions are met. This element is necessary to handle cases in which the category number is inappropriate, or when the input value is missing or otherwise abnoral in the input. The nested **xsl:attribute** node assigns a value of the text string **"NA"** in this case.
         
    + The next **xsl:choose** element controls for the grammatical category of **degree**. 
         + **xsl:when test="&#36;degree='p'"** returns TRUE when the value of the **degree** variable is "**p**", the symbol for **positive** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string. 
         + **xsl:when test="&#36;degree='c'"** returns TRUE when the value of the **degree** variable is "**c**", the symbol for **comparative** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string. 
         + **xsl:when test="&#36;degree='s'"** returns TRUE when the value of the **degree** variable is "**s**", the symbol for **superlative** in the AGLDT. The relevant attribute is created and populated by the nested **xsl:attribute** element and its nested text string. 
         + **xsl:otherwise** returns TRUE when none of the previous conditions are met. This element is necessary to handle cases in which the category number is inappropriate, or when the input value is missing or otherwise abnoral in the input. The nested **xsl:attribute** node assigns a value of the text string **"NA"** in this case.
         

#### Notes on attributes derived from parent of input node. 

* The first step is to set a variable to identify the parent of the current node.
     + The parent is found via the "head" attribute.
         + **xsl:variable name="parent_id" select="./@head"** creates a variable named "parent_id" and gives it the value specified in the **select** attribute, namely the value of the **head** attribute of the current node.
         + **xsl:variable name="parent-pos" select="substring(../word[@id = &#36;parent_id]/@postag, 1, 1)"** creates a variable with the part of speech symbol of the parent of the current node. The variable is named "parent-pos".
             + The first argument of the **substring( )** function is an XPath expression which specifies the syntactic parent (not the XML parent) of the input word: 
                 + in XPath, the **dot dot** indicates the parent XML node of the input node. Here, the parent is the sentence node in which the input word is nested. 
                 + The next part of the XPath expression (parts are separated by the **/** symbol) is **word[@id = &#36;parent_id]**, which, through the **[...]** operator, specifies a particular word in the current sentence, namely that word whose **id** attribute has the value matching the **parent_id** variable. Recall that this varible gets its value from the **head** attribute of the input word. Thus, it identifies the syntactic parent of the input word.
                 + **@postag**, the final part of the XPath expression, selects the **postag** attribute of the node indicated by the expression.
    + Note that the other morphological variables for the syntactic parents are generated analogously to the "self-" variables detailed above. However, there are some important additions to the code.  
        + The **xsl:choose** element differentiates treatment according to whether the **head** attribute of the input word is equal to or greater than 0.
            + **xsl:when test="&#36;parent_id = 0"** identifies the **ROOT** of the sentence. Conventionally, the highest node in a dependency tree is considered the root, and, paradoxically, is depicted as depending on a sentence-external root. Thus, if the **head** attribute is 0, the only relevant feature applicable to its syntactic parent is the designation of its **relation** attribute as "root".
               + **xsl:attribute name="parent-relation"** creates the attribute **parent-relation** and the nested text string adds the value "root".
               + All other **parent-** attributes are given the value "null" to indicate that the parent node essentially does not exist.
           + **xsl:when test="&#36;parent_id > 0"** selects all nodes in the sentence which are not the sentence root. The nested code parallels that for the **self-** attributes and needs no detailed discussion.
     
        
#### Notes on attributes derived from syntactic grandparent of input node. 


* The first step is to set a variable to identify the parent of the current node.
     + The parent is found via the "head" attribute.
         + **xsl:variable name="g1-parent_id"select="parent::sentence/word[@id = &#36;parent_id]/@head"** creates a variable named **g1-parent_id** whose value is produced from the evaluation of the expression in the **select** attriute. Here, the value represents the value of the **head** attribute of the syntactic parent token of the input token. In other words, the value is the **id** attribute of the syntactic grandparent.
             + **parent::sentence**, the first element of the XPath expression, is a so-called "axis" denotation of the parent of the input node, here the matrix sentence.
             + **word[@id = &#36;parent_id]**, the second element of the XPath expression, selects the node which is the syntactic parent of the input word.
             + **@head"**, the last element in the XPath expression, selects the **head** attribute of the syntactic parent. 
     + This variable is used to extract the relevant values from the syntactic grandparent. The values are stored in **g1-parent-** attributes. The process is analogous to that creating the **self-** attributes and will not be discussed in detail. However, there are some important additions to the code. 
         + The **xsl:choose** element differentiates treatment according to whether the **head** attribute of the input word is equal to or greater than 0, or neither.
             + **xsl:when test="&#36;g1-parent_id = 0"** selects a syntactic grandparent of the input word which is the root node of the sentence and assigns appropriate values. These values are the same as discussed above under the **parent-** attributes.
             + **xsl:when test="&#36;g1-parent_id > 0"** selects a syntactic grandparent which is a dependent word. The nested code creates **g1-parent-** variables and assigns the appropriate values.
             + **xsl:otherwise** catches those cases for which a synctactic grandparent does not exist.  This step is first necessary for syntactic grandparents and more distance ancestors. All words in a sentence have syntactic parents except the sentence root, for which a fictitious parent is conventionally supplied. In contrast, many words do not have syntactic grandparents.
                   

#### Notes on attributes derived from syntactic great-grandparent of input node and beyond

* Features from syntactic great-grandparent and more distant ancestors are treated somewhat differently. In particular, **part of speech is the only morphological feature extracted from ancestors of this distance.** The goal of a later stage of the analysis is to use all combinations of variables to characterize texts. With 12 variables per generation, the number of possible combinations quickly outstrips computing resources. Thus, limiting variables is necessary, and the most reasonable place to cut variables is in the distant syntactic ancestors  since variables extracted from these levels most often have values of "null" or "NA".
    + The only variables extracted for these distant syntactic are from the following attributes: **relation,** **DepDist,** **Degree,** and **postag** (part of speech only).
    + Even these limited variables are only applicable when the **head** attribute of the target ancestor is greater than 0.

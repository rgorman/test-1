...
for (i in seq_along(files.v)) {
  ...
  for (j in seq_along(sentence.list)) {
    ...
    node.list <- vector("list", 13)
    names(node.list) <- c("Ellipsis", "Subtree_eligibility", "Subtree", 
                          "DepDist", "Neighborhood", "Degree", "Node_Type",
                          "parent_order", "has_sib", "sib_count",
                          "has_sib_before", "has_sib_after",  "node_depth")
    ...
    node.list$sib_count <- if (not(is.null(edge.graph))) {sapply(sent_working, check_sibling_count)}
    
    node.list$has_sib_before <- if (not(is.null(edge.graph))) {sapply(sent_working, check_sibling_before)}
    
    node.list$has_sib_after <- if (not(is.null(edge.graph))) {sapply(sent_working, check_sibling_after)}

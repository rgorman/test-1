...
sampled_list_of_tibbles <- vector(mode = "list", length(list_of_tibbles))

start_time <- Sys.time()
for(i in seq_along(sampled_list_of_tibbles)) {
  
  sampled_rows <- sample(nrow(list_of_tibbles[[i]]), 1000)
  sampled_list_of_tibbles[[i]] <- list_of_tibbles[[i]][sampled_rows, ] 
...
culled_columns <- col_totals[which(col_totals >= 350) ]
...
saveRDS(culled_names, file = "Feb-23-2018_culed_names.rds")
